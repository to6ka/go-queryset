# fork of [github.com/jirfag/go-queryset](https://github.com/jirfag/go-queryset)

# go-queryset [![GoDoc](https://godoc.org/github.com/to6ka/go-queryset?status.png)](http://godoc.org/github.com/to6ka/go-queryset) [![Go Report Card](https://goreportcard.com/badge/github.com/to6ka/go-queryset)](https://goreportcard.com/report/github.com/to6ka/go-queryset) [![Codacy Badge](https://api.codacy.com/project/badge/Grade/04f432950cbc4e8cb0ec4ef36f4e90bb)](https://www.codacy.com/app/to6ka/go-queryset?utm_source=github.com&amp;utm_medium=referral&amp;utm_content=to6ka/go-queryset&amp;utm_campaign=Badge_Grade) [![Build Status](https://travis-ci.org/to6ka/go-queryset.svg?branch=master)](https://travis-ci.org/to6ka/go-queryset) [![Coverage Status](https://coveralls.io/repos/github/to6ka/go-queryset/badge.svg?branch=master)](https://coveralls.io/github/to6ka/go-queryset?branch=master)
...
# Installation
```bash
go get -u github.com/to6ka/go-queryset/cmd/goqueryset
```

# Usage
## Define models
Imagine you have model `User` in your [`models.go`](https://github.com/to6ka/go-queryset/blob/master/examples/comparison/gorm1/gorm1.go#L16) file:
...
Now transform it by [adding comments for query set generation](https://github.com/to6ka/go-queryset/blob/master/examples/comparison/gorm4/gorm4.go#L15):
```go
...
And you will get file [`autogenerated_models.go`](https://github.com/to6ka/go-queryset/blob/master/examples/comparison/gorm4/autogenerated_gorm4.go) in the same directory (and package) as `models.go`.
...
See full autogenerated file [here](https://github.com/to6ka/go-queryset/blob/master/examples/comparison/gorm4/autogenerated_gorm4.go).
...
```
Under the hood `Create` method [just calls `db.Create(&u)`](https://github.com/to6ka/go-queryset/blob/master/examples/comparison/gorm4/autogenerated_gorm4.go#L38).
...
Goqueryset generates DB names for struct fields into [`UserDBSchema`](https://github.com/to6ka/go-queryset/blob/master/examples/comparison/gorm4/autogenerated_gorm4.go#L414) variable.
In this example we used [`UserDBSchema.Rating`](https://github.com/to6ka/go-queryset/blob/master/examples/comparison/gorm4/autogenerated_gorm4.go#L419).
...
# Golang version
Golang >= 1.7 is required. Tested on go 1.7, 1.8, 1.9 versions by [Travis CI](https://travis-ci.org/to6ka/go-queryset)
...
## Why not any ORM?
I didn't see any ORM that properly handles code duplication. GORM is the best with `Scopes` support, but even it's far from ideal. E.g. we have GORM and [next typical code](https://github.com/to6ka/go-queryset/blob/master/examples/comparison/gorm1/gorm1.go#L16):
```go
type User struct {
	gorm.Model
	Rating      int
...
How to solve it? First idea is to [make reusable functions](https://github.com/to6ka/go-queryset/blob/master/examples/comparison/gorm2/gorm2.go#L27):
```go
...
We can use GORM [Scopes](http://jinzhu.me/gorm/crud.html#scopes) to improve [how it looks](https://github.com/to6ka/go-queryset/blob/master/examples/comparison/gorm3/gorm3.go#L64):
```go
...
Now compare it with [go-queryset solution](https://github.com/to6ka/go-queryset/blob/master/examples/comparison/gorm4/gorm4.go#L30):
```go
...
* QuerySet pattern allows to reuse queries by defining [custom methods](https://github.com/to6ka/go-queryset/blob/master/examples/comparison/gorm4/gorm4.go#L30) on it.
